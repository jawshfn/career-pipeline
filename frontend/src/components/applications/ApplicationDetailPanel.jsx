...
const initialFormState = {
  ...
  red_flags_notes: "",
  furthest_stage: SAVED_APPLICATION_STATUS,
};
...
function toFormState(application) {
  return {
    ...
    furthest_stage: application.furthest_stage || SAVED_APPLICATION_STATUS,
  };
}
...
export default function ApplicationDetailPanel({
  ...
  const [isCorrectingHistory, setIsCorrectingHistory] = useState(false);
  ...
  async function confirmOutcomeHistoryCorrection() {
    if (isCorrectingHistory || confirmedStage === savedFormData.furthest_stage) return;
    setIsCorrectingHistory(true);
    setSaveError("");
    try {
  ...
  return (
    <section className="panel application-detail-panel" aria-labelledby="application-detail-title">
          ...
          >
            {activeTab === "overview" ? (
              <ApplicationDetailOverview
                ...
                canCorrectOutcomeHistory={savedFormData.status !== "Archived"}
                furthestStage={savedFormData.furthest_stage}
                onCorrectOutcomeHistory={() => {
                  setConfirmedStage(savedFormData.furthest_stage);
      ...
      {isCorrectionDialogOpen ? <ConfirmationDialog cancelLabel="Cancel" confirmLabel="Correct history" confirmDisabled={confirmedStage === savedFormData.furthest_stage} description={<div className="outcome-history-correction-content"><dl className="outcome-history-current-summary"><div className="outcome-history-current-item"><dt>Current status</dt><dd>{savedFormData.status}</dd></div><div className="outcome-history-current-item"><dt>Highest confirmed stage</dt><dd>{savedFormData.furthest_stage}</dd></div></dl><p>This changes historical outcome reporting only; it does not change the current status.</p><label className="outcome-history-stage-field">Highest confirmed stage<select value={confirmedStage} onChange={(event) => setConfirmedStage(event.target.value)}>{outcomeStages.filter((stage) => stageRank(stage) >= Math.max(0, stageRank(savedFormData.status))).map((stage) => <option key={stage} value={stage}>{stage}</option>)}</select><small>Select the furthest stage this application actually reached.</small></label><p className="outcome-history-correction-note">Saving this correction adds an entry to the application activity timeline.</p></div>} descriptionClassName="outcome-history-correction-description" dialogClassName="outcome-history-correction-dialog" isOpen isProcessing={isCorrectingHistory} processingLabel="Correcting..." title="Correct outcome history" onCancel={() => !isCorrectingHistory && setIsCorrectionDialogOpen(false)} onConfirm={confirmOutcomeHistoryCorrection} /> : null}
