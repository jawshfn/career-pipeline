...
const renderBrief = (props = {}) => renderToStaticMarkup(<JobIntelligenceBriefTab brief={null} eligibility={eligible} error="" isGenerating={false} isStale={false} meta={null} onGenerate={() => {}} {...props} />);
...
describe("JobIntelligenceBriefTab", () => {
  ...
  it("keeps a report visible while refreshing and places stale status before it", () => {
    const markup = renderBrief({ brief, meta: generatedMeta, isGenerating: true, isStale: true, error: "Unavailable" });
    ...
    expect(markup).toContain("Refresh it to update the analysis.");
    expect(markup.indexOf("Refresh it to update the analysis.")).toBeLessThan(markup.indexOf("Own platform reliability."));
  });
});
