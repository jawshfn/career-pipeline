...
This experimental, locally loaded Chrome extension supports verified Greenhouse identifier capture and click-initiated Indeed, LinkedIn, and experimental ZipRecruiter text capture into an editable local PursuitHQ review.
...
For Indeed, LinkedIn, and ZipRecruiter, the helper reads only the current visible job after the user clicks the extension. It sends bounded cleaned text only after **Open in PursuitHQ** is selected. The local backend keeps that text in memory for at most two minutes and consumes it once into **Add Job** -> **Paste Job Text**. LinkedIn supports search-results current-job side panels and standalone `/jobs/view/{id}` pages. ZipRecruiter capture is experimental and pending live QA; it currently targets signed-in `/jobs-search` pages with exactly one `lk` selected-job key. The extension makes no request to job boards and has no job-board host permissions. Captures are not written to SQLite unless the user reviews and explicitly saves.
...
- Indeed and LinkedIn capture each support one confidently detected current job only. LinkedIn supports search-results current-job side panels and standalone `/jobs/view/{id}` pages. ZipRecruiter support is limited to the experimental signed-in selected-job `/jobs-search` detail pane; standalone and signed-out ZipRecruiter layouts are not supported. Handshake, generic pages, and selected text are not supported.
