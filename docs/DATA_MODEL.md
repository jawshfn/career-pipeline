...
`furthest_stage` is the highest confirmed stage the application genuinely reached. It uses `Saved`, `Applied`, `Assessment`, `Recruiter Screen`, `Interview`, and `Offer`. Saved means not submitted; confirmed Saved history clears `date_applied`. Moving backward corrects the current and confirmed stage together. Rejected and Withdrawn preserve confirmed reach while closed, while reopening below that reach corrects it to the reopened status. The separate outcome-history correction can repair permanent history independently, but never below an active current status. Normal reporting reads this persisted history rather than parsing Activity prose.
