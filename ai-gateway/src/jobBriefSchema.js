  ...
  properties: {
    ...
    interview_preparation: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["topic", "preparation"],
        properties: { topic: text(400), preparation: text(600) },
      },
    },
...
function failure(path, code) {
  return { brief: null, issue: { path, code } };
}

function success(brief) {
  return { brief, issue: null };
}

function objectKeys(value, keys, path) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return failure(path, "wrong_type");
  for (const key of keys) {
    if (!(key in value)) return failure(`${path}.${key}`, "missing_key");
  }
...
export function validateJobBriefDetailed(value) {
  ...
  const strings = (field, minItems, maxItems) =>
    arrayValue(value[field], minItems, maxItems, `$.${field}`, (item, path) => textValue(item, 600, path));
  const roleSummary = textValue(value.role_summary, 1000, "$.role_summary");
  const responsibilities = strings("responsibility_themes", 0, 7);
  const formal = strings("formal_requirements", 0, 8);
  const preferred = strings("preferred_qualifications", 0, 6);
  const conditions = strings("important_conditions", 0, 6);
  const skills = strings("skills_and_tools", 0, 12);
  const interview = arrayValue(value.interview_preparation, 0, 6, "$.interview_preparation", (item, path) =>
    itemValue(item, ["topic", "preparation"], path),
  );
  const research = strings("research_questions", 1, 5);
  const unknowns = strings("unknowns", 1, 5);
  const nextAction = itemValue(value.next_action, ["action", "reason"], "$.next_action");
  const limitations = strings("limitations", 1, 3);
  ...
  return success({
    ...
    role_summary: roleSummary.brief,
    responsibility_themes: responsibilities.brief,
    formal_requirements: formal.brief,
    preferred_qualifications: preferred.brief,
    important_conditions: conditions.brief,
    skills_and_tools: skills.brief,
    interview_preparation: interview.brief,
    research_questions: research.brief,
    unknowns: unknowns.brief,
    next_action: nextAction.brief,
    limitations: limitations.brief,
  });
}
